...
This paper is carried out using statistical programming language `R` [@citeR], the library `tidyverse` [@tid] for preparation and analysis of data, the `janitor` [@jan], the `here` [@here] for read data, the `ggplot2` for generating figures, the `dplyr` [@dp], and we run the model in `R` [@citeR] using the `rstanarm` package [@rstanarm], the `boom` package [@boom], and the `boom.Mixed` package [@bm]. Since this paper is a reproduction of 'Telling Stories with Data' [@alexander_2023], some similar code were used.
